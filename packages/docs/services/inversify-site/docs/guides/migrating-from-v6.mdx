...
`Container.rebind` has been updated to `Container.rebindSync`. The `rebind` method is now asynchronous and returns a promise. The `rebindSync` method is a synchronous version of the `rebind` method that will throw an error if the operation would trigger an asynchronous deactivation.

### unbind and unbindAll methods

`Container.unbind` has been updated to `Container.unbindSync`. The `unbind` method is now asynchronous and returns a promise. The `unbindSync` method is a synchronous version of the `unbind` method that will throw an error if the operation would trigger an asynchronous deactivation.
